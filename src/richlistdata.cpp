		      ...
		      if(!fInvalid)
		      {
			    ...
			    { 
			        ...
			        if(ExtractDestination(atxout.scriptPubKey, dest))
			        {
			    	        ...
			    	        std::string craveAddress = CBitcoinAddress(dest).ToString();
				        CRichListDataItem richListDataItem;
				        richListDataItem.sAddress = craveAddress;
				        if(richListData.mapRichList.find(craveAddress) == richListData.mapRichList.end())
				            richListData.mapRichList.insert(make_pair(craveAddress, richListDataItem));
					
				        richListData.mapRichList[craveAddress].nBalance = richListData.mapRichList[craveAddress].nBalance - atxout.nValue;
