...
AddEditCraveNode::AddEditCraveNode(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::AddEditCraveNode)
{
    ui->setupUi(this);

}

AddEditCraveNode::~AddEditCraveNode()
{
    delete ui;
}


void AddEditCraveNode::on_okButton_clicked()
{
	...
	bool bForceNew = false;
        ...
        pwalletMain->mapMyCraveNodes.insert(make_pair(c.sAddress, c));
	walletdb.WriteCraveNodeConfig(c.sAddress, c);
        uiInterface.NotifyCraveNodeChanged(c);

        accept();
    }
}

void AddEditCraveNode::on_cancelButton_clicked()
{
    reject();
}
